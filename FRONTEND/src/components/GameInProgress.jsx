...
const GameInProgress = () => {
    ...
    return (
      !gameInProgress ? (
        <button onClick={handleStartGame} type="button"
          className="h-12 w-32 text-white bg-gradient-to-r from-purple-500 via-purple-600 to-purple-700 hover:bg-gradient-to-br focus:ring-4 
                    focus:outline-none focus:ring-purple-300 dark:focus:ring-purple-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2"
        >
          START
        </button>
      ) : (
  ...
  return (
    <div className='grid grid-cols-3 grid-rows-5 gap-4 p-4 h-screen gradient-blue'>
      ...
      <div className='col-span-3 row-span-4 flex p-4 rounded-2xl h-full justify-center items-center h-full bg-red-100'>
        {
          renderButtonGame()
        }
      </div>
