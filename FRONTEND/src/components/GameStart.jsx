        ...
        return (
            <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
                <div className="bg-white w-1/2 p-8 rounded-2xl shadow-lg">
                        ...
                        <div className="pl-4">
                            {
                                questionGameIn.answer.map((a, index) => {
                                    return(
                                        <div>
                                            <p key={index} className="mb-2">{a.response}</p>
                                        </div>
                                    )
                                })
                            }
                    ...
                    <button className='py-2 px-4 bg-blue-500 text-white rounded hover:bg-blue-600' onClick={()=>setModalQuestion(false)}>Cerrar</button>
                </div>
            </div>
        );
    }
                    ...
                    categorys && categorys.map((c) => {
                        return (
                            <div key={c.id_category} className='col-span-1 row-span-1'>
                                <button type="button"
                                    className="w-full  h-full bg-white rounded-2xl shadow shadow-2xl">
                                    ...
                                    dataQuestions.map((q,ind) => {
                                        if (q.CategoryIdCategory == categorys[index].id_category) {
                                            return (
                                                ...
                                                font-bold text-gray-600 bg-white row-span-1 rounded-2xl shadow shadow-2xl hover:cursor-pointer">
